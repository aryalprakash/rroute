...
if ($interactions){
	foreach ($interactions as $i){
		...
		<?php if ($option_settings=='2' && $limit_settings=='4'||$option_settings=='1' && $limit_settings=='4') { ?>
		 ...
		 else if ($option_settings=='2' && $limit_settings=='1') { ?>
			<div style="font-size:15px; margin-bottom:10px;" >
				<img src="<?php echo $icon_image?>" width="20px">
				<span class=""><?php echo ucwords($user).' '.$action.' '?><a href="<?php echo SITE_URL;?>/user.php?uid=<?php echo  $i['author'];?>" style="color: #FF0303; text-decoration:none;"><?php echo ucwords($author);?></a><?php echo "'s ". $i['type']?> <a href="<?php echo $url?>" style="color: #FF0303; text-decoration:none;"><?php echo $title;?>.<a></span>
				<span style="margin-left:10px; font-size:13px;"><?php echo TimeAgo(date('Y-m-d', strtotime($timestamp))); ?> </span>
		...
		<?php }
		else if ($option_settings=='2' && $limit_settings=='2') { ?>
			<?php 	if($_GET['uid']==$_SESSION['uid']) { ?>
				<div style="font-size:15px; margin-bottom:10px;" >
					<img src="<?php echo $icon_image?>" width="20px">
					<span class=""><?php echo ucwords($user).' '.$action.' '?><a href="<?php echo SITE_URL;?>/user.php?uid=<?php echo  $i['author'];?>" style="color: #FF0303; text-decoration:none;"><?php echo ucwords($author);?></a><?php echo "'s ". $i['type']?> <a href="<?php echo $url?>" style="color: #FF0303; text-decoration:none;"><?php echo $title;?>.<a></span>
		...
		<?php }
		...
		else if ($option_settings=='2' && $limit_settings=='3') { ?>
			<?php  if($_GET['uid']==$_SESSION['uid']) { ?>
				<div style="font-size:15px; margin-bottom:10px;" >
					<img src="<?php echo $icon_image?>" width="20px">
					<span class=""><?php echo ucwords($user).' '.$action.' '?><a href="<?php echo SITE_URL;?>/user.php?uid=<?php echo  $i['author'];?>" style="color: #FF0303; text-decoration:none;"><?php echo ucwords($author);?></a><?php echo "'s ". $i['type']?> <a href="<?php echo $url?>" style="color: #FF0303; text-decoration:none;"><?php echo $title;?>.<a></span>
			...
			<?php }
				...
				//print_r($routers_router);
						if(!empty($routers)){
								foreach ($routers as $router){
									//=getRoutersForUser($router['user_id']);
									//print_r($routers_router);
							...
							foreach ($routers_router as $routed) {
								...
								foreach ($routersrouter as $rout) {
									if ($rout['user_id'] == $_GET['uid']) { ?>
										<div style="font-size:15px; margin-bottom:10px;">
											<img src="<?php echo $icon_image ?>" width="20px">
											<span class=""><?php echo ucwords($user) . ' ' . $action . ' ' ?><a
														href="<?php echo SITE_URL; ?>/user.php?uid=<?php echo $i['author']; ?>"
														style="color: #FF0303; text-decoration:none;"><?php echo ucwords($author); ?></a><?php echo "'s " . $i['type'] ?>
												<a href="<?php echo $url ?>"
												   style="color: #FF0303; text-decoration:none;"><?php echo $title; ?>
													.<a></span>
											<span style="margin-left:10px; font-size:13px;"><?php echo TimeAgo(date('Y-m-d', strtotime($timestamp))); ?> </span>
										</div>
									<?php }
