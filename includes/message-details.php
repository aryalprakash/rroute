<div class="inbox-messages">
    ...
    if ($participants['sender'] == $_SESSION['uid'] || $participants['recipient'] == $_SESSION['uid']) {
        ...
        foreach ($messages as $message) {
            ...
            if($user['user_id']==$_SESSION['uid']){$classval='router-user-photo answer-photo';
                $activeornot='even';
                $aligning='right';
            }
            else{
                    ...
                    <div class="<?php echo $classval; ?>">
                        <a href="<?php echo SITE_URL ?>/user.php?uid=<?php echo $message['sender']; ?>">
                            <?php if (empty($user['photo'])) { ?>
                                <img src="uploads/avatars/nophoto.jpg" alt="<?php $dates=strtotime($message['created_on']); echo date("g:i A m/d/Y",$dates); ?>">
                            <?php } else { ?>
                                <img src="uploads/avatars/<?php echo $user['photo']; ?>" alt="<?php $dates=strtotime($message['created_on']); echo date("g:i A m/d/Y",$dates); ?>">
                            <?php } ?>
                        </a>
...
<!--                                  style="margin-left: 0;">--><?php //$dates=strtotime($message['created_on']); echo date("g:i A m/d/Y",$dates); ?><!--</span>-->
                        </div>
                    </div>
                </div>
        ...
        <div class="message-item" style="border-top: none;"><?php
            ...
            <div class="answer-box" id="answer_<?php echo $_GET['conv']; ?>">
                ...
                <div class="router-user-photo answer-photo">
                    <a href="user.php?uid=<?php echo $own['user_id']; ?>">
                        <?php if (empty($own_photo)) { ?>
                            <img src="uploads/avatars/nophoto.jpg" alt="<?php $dates=strtotime($message['created_on']); echo date("g:i A m/d/Y",$dates); ?>">
                        <?php } else { ?>
                            <img src="uploads/avatars/<?php echo $own_photo; ?>" alt="<?php $dates=strtotime($message['created_on']); echo date("g:i A m/d/Y",$dates); ?>">
                        <?php } ?>
                    </a>
                ...
                <input class="submit-rounded answer-button" type="submit" value="Send" name="save_account"
                       ...
                       data-user="<?php echo getMessageRecipient($_GET['conv']); ?>" data-block="" method="post">
